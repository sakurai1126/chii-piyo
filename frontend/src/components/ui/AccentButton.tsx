...
const variantStyles = {
  ...
  cancel:
    "border-line-gray mx-auto mt-5 flex h-10 w-fit cursor-pointer items-center justify-center gap-3 rounded-lg border bg-white px-7 text-sm font-medium transition-all duration-300 hover:bg-gray-100 max-md:gap-2 max-md:px-3 max-md:text-xs",
};
...
export const AccentButton = ({ styleVariant = "primary", ...props }: Props) => {
  if (props.variant === "button") {
    ...
    return (
      <button {...buttonProps} className={`${variantStyles[styleVariant]} ${className ?? ""}`}>
        {children}
      </button>
    );
  }
  ...
  if (props.variant === "link") {
    ...
    return (
      <Link href={href} className={`${variantStyles[styleVariant]} ${className ?? ""}`}>
        {children}
      </Link>
    );
  }
  ...
  if (props.variant === "a") {
    const { children, className, href } = props;
    return (
      <a href={href} className={`${variantStyles[styleVariant]} ${className ?? ""}`}>
        {children}
      </a>
    );
  }
