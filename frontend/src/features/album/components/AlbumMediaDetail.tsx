...
export const AlbumMediaDetail = ({ isAdmin, isEasy, album, media }: Props) => {
  ...
  return (
    <div className="mt-7">
      <p className={cn("@max-md:text-sm", isEasy && "font-medium @max-md:text-lg")}>アルバム</p>
      <div className="border-brown-dark bg-translucent mt-2 flex justify-between rounded-lg border p-4 backdrop-blur-[7.5px] @max-md:flex-col">
        <div className="flex gap-3">
          <Image
            ...
            width={80}
            height={80}
            className={cn("aspect-square h-20 w-20 rounded-sm object-cover", isEasy && "h-30 w-30")}
          />
          <div className={cn(isEasy && "flex flex-col justify-between")}>
            <div>
              <p
                className={cn(
            ...
            {isEasy && (
              <a href={`/albums/${album.id}`}>
                <Button className="w-32 @max-md:h-8 @max-md:w-30" isEasy={isEasy}>
                  アルバムを見る
                </Button>
              </a>
            )}
