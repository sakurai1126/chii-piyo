...
export const MilkAction = () => {
  ...
  const milkMinus = () => {
    if (amountMl <= 10) {
      toast.error("これ以上減らすことはできません");
      return;
    }
    setAmountMl(amountMl - 10);
  };

  // プラスボタンを押したときの処理
  const milkPlus = () => {
    if (amountMl >= 400) {
      toast.error("これ以上増やすことはできません");
      return;
    }
    setAmountMl(amountMl + 10);
  };
    ...
    <div className="max-md:w-[calc(50%-8px)]">
      ...
      >
        <div className="mt-4 flex items-center gap-4 max-md:justify-center">
          ...
          <div>
            <span className="text-4xl font-medium">{amountMl}</span>
