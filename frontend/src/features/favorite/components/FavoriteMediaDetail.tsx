...
export const FavoriteMediaDetail = ({ media, users }: Props) => {
  ...
  const [isFavorite, setIsFavorite] = useState<boolean>(media.isFavorite || false);

  // 前回値を保持するステート
  const [prevServerFavorite, setPrevServerFavorite] = useState<boolean | undefined>(
    media.isFavorite,
  );

  // 更新時に変化を検知して更新する
  if (media.isFavorite !== prevServerFavorite) {
    setPrevServerFavorite(media.isFavorite);
  ...
  const favoriteToggle = () => {
    startTransition(async () => {
      if (isFavorite) {
        const result = await removeFavoriteAction({ mediaId: media.id });
        if (result.success) {
          setIsFavorite(false);
        ...
        const result = await addFavoriteAction({ mediaId: media.id });
        if (result.success) {
          setIsFavorite(true);
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <button className="group" onClick={favoriteToggle} disabled={isPending}>
        <svg width="30" height="30" viewBox="0 0 25 25" xmlns="http://www.w3.org/2000/svg">
          <path
            ...
            className={`stroke-brown-dark transition-all ${isFavorite ? "fill-accent-pink group-hover:fill-accent-pink/80" : "group-hover:fill-accent-pink/20 fill-white"}`}
          />
        </svg>
      </button>
    </div>
