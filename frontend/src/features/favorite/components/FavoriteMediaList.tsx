...
export const FavoriteMediaList = ({ media, users }: Props) => {
  ...
  return (
    <div className="absolute top-2 right-2 flex items-center gap-0.5 @max-md:top-1 @max-md:right-1">
      ...
      <button
        type="button"
        className="group/favorite"
        onClick={(e) => favoriteToggle(e)}
        disabled={isPending}
      >
        <svg width="25" height="25" viewBox="0 0 25 25" xmlns="http://www.w3.org/2000/svg">
          <path
            ...
            className={cn(
              ...
              media.isFavorite
                ? "fill-accent-light-pink group-hover/favorite:fill-accent-light-pink/80"
                : "group-hover/favorite:fill-accent-light-pink/20 fill-white",
            )}
          />
        </svg>
      </button>
