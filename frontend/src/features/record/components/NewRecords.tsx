...
export const NewRecords = ({ tags, sharingGroups, variant }: Props) => {
  ...
  return (
    <div className="border-brown-dark bg-background-light w-full rounded-lg border border-dashed max-md:mt-6">
      {/* 開くボタン */}
      <AccordionContent isOpen={!isMenuOpen}>
        <button
          className={`hover:bg-background-light bg-green-back flex w-full cursor-pointer items-center justify-center gap-3 rounded-lg transition-all ${variant === "word" ? "h-30" : "h-20"}`}
