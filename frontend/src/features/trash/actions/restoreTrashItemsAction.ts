...
export const restoreTrashItemsAction = async (input: Input): Promise<ActionResult> => {
  try {
    ...
    console.error("restoreTrashItemsAction失敗", error);
    if (error instanceof Error && error.message === "UNAUTHORIZED") {
      return { success: false, error: "認証が必要です" };
    }
    return { success: false, error: "メディアの復元に失敗しました" };
  }
};
